...
hk_genes <-readRDS("/home/yuxuan.wu/meta_analysis/HK_genes.rds") 

## huamn: m6a site in experiment 
#IP/input identify RNA methylation site by MeRIP
Methylation_level <- readRDS("/home/kunqi/meta_analysis/human_meRIP_Level.rds")

## add the GSE46705 Mettle3 knockdown and control comparison 
mcols(human) <- Methylation_level[,c(8:9)]

human_exon <- subsetByOverlaps(human,hk_genes) ## find the exon site that could be expressed methylation
...
###next will be the control sample

# find the DRACH sequence motif other than the experiment one 
# use to extract the regions of a query sequence mapped to under defined sub-regions of genome
# find all the sequence under the circumstance of DRACH
motif <- m6ALogisticModel::sample_sequence("DRACH",subsetByOverlaps(hk_genes,human),Hsapiens)
motif <- motif-2
motif <- motif[-which(motif%in%human_exon)]

# mapped information with IP/input motif excluded
set.seed(213)

...
## add the GSE90642 Mettle3 knockdown and control comparison 
mcols(human) <- Methylation_level[,c(48:49)]
...
Ctrl_peak <- read.csv("/home/kunqi/human_peakCalling/GSE90642/HepG2_Ctrl/Mod.csv")
...
M14KO_peak <- read.csv("/home/kunqi/human_peakCalling/GSE90642/HepG2_METTL14KD/Mod.csv")
...
ctrl_pos <- length(which(c(background_motif$Ctrl-background_motif$M14KO)>0))
ctrl_neg <- length(which(c(background_motif$M14KO-background_motif$Ctrl)>=0))
